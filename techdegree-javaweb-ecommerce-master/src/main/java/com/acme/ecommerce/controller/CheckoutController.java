...
public class CheckoutController {
    ...
    String postShipping(@ModelAttribute(value = "shippingAddress") @Valid Address shippingAddress, final
	BindingResult result, RedirectAttributes redirectAttributes) {

        if (result.hasErrors()) {
            logger.error("Errors on fields: " + result.getFieldErrorCount());
            redirectAttributes.addFlashAttribute("org.springframework.validation.BindingResult.shippingAddress",
					result);
    ...
    String checkoutConfirmation(Model model) {
        ...
        String number = purchase.getCreditCardNumber();
        String maskCC = number.replaceAll("\\w(?=\\w{4})", "*");
        ...
        if (purchase != null) {
            ...
            model.addAttribute("creditCard", maskCC);
        } else {
            logger.error("No purchases Found!");
            return ("redirect:/error");
        }
