...
public class CartController {
    ...
    public RedirectView addToCart(@ModelAttribute(value = "productId") long productId, @ModelAttribute(value =
            "quantity")
        ...
        if (addProduct != null) {
            ...
            redirectAttributes.addFlashAttribute("flash", new FlashMessage("Added " + quantity + " of " + addProduct
                    .getName() + " to cart", SUCCESS));
    ...
    public RedirectView updateCart(@ModelAttribute(value = "productId") long productId, @ModelAttribute(value =
            "newQuantity")
                    ...
                    if (pp.getProduct() != null) {
                        if (pp.getProduct().getId().equals(productId)) {
                            if (newQuantity > 0) {
                                ...
                                redirectAttributes.addFlashAttribute("flash", new FlashMessage("Updated " +
                                        updateProduct.getName() + " to " + newQuantity, SUCCESS));
                            } else {
                                purchase.getProductPurchases().remove(pp);
                                logger.debug("Removed " + updateProduct.getName() + " because quantity was set to " +
                                        newQuantity);
                                redirectAttributes.addFlashAttribute("flash", new FlashMessage("Removed " +
                                        updateProduct.getName() + " because quantity was set to " + newQuantity,
                                        SUCCESS));
                            }
                            break;
                        }
    ...
    public RedirectView removeFromCart(@ModelAttribute(value = "productId") long productId, RedirectAttributes
            redirectAttributes) {
                ...
                for (ProductPurchase pp : purchase.getProductPurchases()) {
                    if (pp.getProduct() != null) {
                        if (pp.getProduct().getId().equals(productId)) {
                            ...
                            redirectAttributes.addFlashAttribute("flash", new FlashMessage("Removed " + updateProduct
                                    .getName(), SUCCESS));
                            break;
                        }
                    }
